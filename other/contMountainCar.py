...
class DDPG:
    def __init__(self, state_shape, action_shape, action_bound, gamma=0.99, tau=0.005, actor_lr=0.001, critic_lr=0.002, update_every=10000):
    ...
    def train(self):
        ...
        with tf.GradientTape() as tape:
            ...
            y = rewards + self.gamma * (1 - dones) * self.target_critic([tf.convert_to_tensor(next_states, dtype=tf.float32), target_actions], training=True)
            critic_value = self.critic([tf.convert_to_tensor(states, dtype=tf.float32), tf.convert_to_tensor(actions, dtype=tf.float32)], training=True)
